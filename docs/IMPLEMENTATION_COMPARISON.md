# SEI CLOB Implementation Comparison with DeepBook and Phoenix

This document compares our SEI CLOB implementation with industry-standard DEXes like DeepBook and Phoenix, focusing on key areas identified during end-to-end testing.
...
### DeepBook (Solana)
- **Complete Matching**: DeepBook uses Solana's architecture to match orders completely across multiple price levels in a single transaction.
...
### Phoenix (Solana)
- **Crankless Design**: Phoenix operates without a "crank" (external trigger for matching), allowing for more efficient order matching.
...
While our SEI CLOB implementation provides a solid foundation with a custody-free design, there are several areas where we can learn from DeepBook and Phoenix to improve our implementation. The key areas for improvement include:
...
While the core functionality is tested, a comparison with the likely testing focus of DeepBook (high throughput) and Phoenix (crankless atomicity) reveals areas for expanded end-to-end test coverage:
